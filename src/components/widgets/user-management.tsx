...
export function UserManagementWidget() {
    ...
    const [kotamaFilter, setKotamaFilter] = useState("Semua");
    const [satminkalFilter, setSatminkalFilter] = useState("Semua");
    ...
    const [newKotamaId, setNewKotamaId] = useState("");
    ...
    const { data: kotamaList = [] } = useQuery({
    ...
    // Filtered satminkals for Super Admin & Kotama Admin
    const availableSatminkalsForForm = useMemo(() => {
        if (isSuperAdmin && newKotamaId) {
            return satminkalList.filter((s: any) => s.kotamaId === newKotamaId);
        }
        if (isKotamaAdmin && sessionKotamaId) {
            return satminkalList.filter((s: any) => s.kotamaId === sessionKotamaId);
        }
        return satminkalList;
    }, [satminkalList, isSuperAdmin, isKotamaAdmin, newKotamaId, sessionKotamaId]);

    const availableSatminkalsForFilter = useMemo(() => {
        if (isSuperAdmin && kotamaFilter !== "Semua") {
            return satminkalList.filter(
                (s: any) => s.kotamaId === kotamaFilter || s.kotama?.nama === kotamaFilter || s.kotama?.id === kotamaFilter
            );
        }
        if (isKotamaAdmin && sessionKotamaId) {
            return satminkalList.filter((s: any) => s.kotamaId === sessionKotamaId);
    ...
    const createUserMutation = useMutation({
        mutationFn: () =>
            apiUsers.create({
                ...
                kotamaId:
                    newRole === "SUPER_ADMIN"
                        ? undefined
                        : newRole === "ADMIN_KOTAMA"
                            ? newKotamaId || kotamaList[0]?.id || undefined
                            : newKotamaId || sessionKotamaId || undefined,
                satminkalId:
                    newRole === "ADMIN_KOTAMA" || newRole === "SUPER_ADMIN"
                        ? undefined
                        : newSatminkalId || sessionSatminkalId || satminkalList[0]?.id || "",
            }),
        onSuccess: (res) => {
        ...
        onError: (err: any) =>
            toast.error("Gagal Membuat User", {
                description: err.message || "Terjadi kesalahan saat membuat pengguna.",
            }),
    });
    ...
    const toggleAktifMutation = useMutation({
        ...
        onSuccess: (res: any) => {
            ...
            toast.success(
                ...
                {
                    description: `Status akun @${res?.username || ""} sekarang: ${isNowActive ? "AKTIF" : "NONAKTIF"
                        }`,
                }
            );
            queryClient.invalidateQueries({ queryKey: ["users-list"] });
    ...
    const rows = useMemo(() => {
        const filtered = userList.filter((u) => {
            ...
            // Kotama filter (for Super Admin)
            let matchKotama = true;
            if (isSuperAdmin && kotamaFilter !== "Semua") {
                const uKotamaId = (u as any).kotamaId || (u as any).kotama?.id || (u as any).satminkal?.kotamaId || (u as any).satminkal?.kotama?.id;
                const uKotamaNama = (u as any).kotama?.nama || (u as any).satminkal?.kotama?.nama;
                matchKotama =
                    uKotamaId === kotamaFilter ||
                    uKotamaNama === kotamaFilter ||
                    (kotamaFilter === "MABES" && u.role === "SUPER_ADMIN");
            }

            // Satminkal filter
            let matchSatminkal = true;
            if (satminkalFilter !== "Semua") {
                const uSatminkalId = (u as any).satminkalId || (u as any).satminkal?.id;
                const uSatminkalNama = (u as any).satminkal?.nama;
                matchSatminkal = uSatminkalId === satminkalFilter || uSatminkalNama === satminkalFilter;
            }
            ...
            const matchSearch =
                ...
                ((u as any).satminkal?.nama || "").toLowerCase().includes(qLower) ||
                ((u as any).kotama?.nama || "").toLowerCase().includes(qLower);

            return matchRole && matchStatus && matchKotama && matchSatminkal && matchSearch;
        });
    ...
    }, [userList, roleFilter, statusFilter, kotamaFilter, satminkalFilter, q, isSuperAdmin]);
    ...
    return (
        <div className="space-y-6">
                ...
                <Card className="shadow-card border-primary/20 bg-card">
                    ...
                    <CardContent>
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5">
                            {isSuperAdmin ? "Seluruh Jajaran TNI AD" : "Tersinkronisasi 100% User & Anggota"}
                        </p>
                    </CardContent>
                </Card>
            ...
            <Card className="shadow-card">
                <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-4">
                    <div>
                        ...
                        <CardDescription className="text-xs mt-0.5">
                            {isSuperAdmin
                                ? "Wewenang penuh CRUD seluruh tingkatan Admin Kotama, Admin Satminkal, dan Personel TNI AD dengan proteksi dialog konfirmasi (Yes/No)."
                ...
                <CardContent className="space-y-4 pt-4">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
                        ...
                        {/* Kotama Filter (Super Admin) */}
                        {isSuperAdmin && (
                            <Select
                                value={kotamaFilter}
                                onValueChange={(val) => {
                                    setKotamaFilter(val);
                                    setSatminkalFilter("Semua");
                                }}
                            >
                                <SelectTrigger className="h-9 text-xs">
                                    <Building2 className="size-3.5 mr-1 text-muted-foreground" />
                                    <SelectValue placeholder="Semua Kotama" />
                                </SelectTrigger>
                                <SelectContent className="text-xs max-h-60">
                                    <SelectItem value="Semua">Semua Kotama / Balakpus</SelectItem>
                                    {kotamaList.map((k) => (
                        ...
                        {/* Satminkal Filter (Super Admin & Kotama Admin) */}
                        {(isSuperAdmin || isKotamaAdmin) && (
                            <Select value={satminkalFilter} onValueChange={setSatminkalFilter}>
                                <SelectTrigger className="h-9 text-xs">
                                    <Layers className="size-3.5 mr-1 text-muted-foreground" />
                                    <SelectValue placeholder="Semua Satminkal" />
                                </SelectTrigger>
                                <SelectContent className="text-xs max-h-60">
                                    <SelectItem value="Semua">Semua Satminkal</SelectItem>
                                    {availableSatminkalsForFilter.map((s) => (
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span>
                            Menampilkan <strong>{rows.length}</strong> dari {userList.length} total personel
                        </span>
                        {(kotamaFilter !== "Semua" || satminkalFilter !== "Semua" || roleFilter !== "Semua" || statusFilter !== "Semua" || q) && (
                            <Button
                                variant="ghost"
                                size="sm"
                                className="h-7 text-xs text-primary"
                                onClick={() => {
                                    setKotamaFilter("Semua");
                                    setSatminkalFilter("Semua");
                                    setRoleFilter("Semua");
                        ...
                        <Table>
                            <TableHeader className="bg-muted/40">
                                <TableRow>
                                    ...
                                    {isSuperAdmin && <TableHead>Kotama / Balakpus</TableHead>}
                                    <TableHead>{isSuperAdmin ? "Satuan / Satminkal" : "Satuan"}</TableHead>
                            ...
                            <TableBody>
                                {isLoading ? (
                                    <TableRow>
                                        <TableCell colSpan={isSuperAdmin ? 9 : 8} className="py-12 text-center text-muted-foreground">
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={isSuperAdmin ? 9 : 8} className="py-12 text-center text-muted-foreground">
                                            Tidak ada personel yang sesuai dengan kriteria filter/pencarian.
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    rows.map((u: any, idx) => {
                                        ...
                                        const kotamaName =
                                            u.kotama?.nama ||
                                            u.satminkal?.kotama?.nama ||
                                            (u.role === "SUPER_ADMIN" ? "MABES TNI AD" : "-");
                                        const satminkalName =
                                            u.satminkal?.nama ||
                                            (u.role === "ADMIN_KOTAMA"
                                                ? "Mako Kotama / Balakpus"
                                                : u.role === "SUPER_ADMIN"
                                                    ? "Pusat Mabesad"
            ...
            {/* ALERT DIALOG: KONFIRMASI RESET PASSWORD */}
            <AlertDialog
                open={openConfirmResetPassword}
                onOpenChange={setOpenConfirmResetPassword}
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Kata sandi untuk personel <strong>{resetPassUser?.namaLengkap}</strong> (@{resetPassUser?.username}) akan digantikan dengan kata sandi baru. Pengguna wajib menggunakan kata sandi baru ini untuk login berikutnya.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        ...
                        >
                            {resetPasswordMutation.isPending ? <Loader2 className="size-4 animate-spin mr-2" /> : null}
                            Iya, Reset Password
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {toggleStatusUser?.isActive || (toggleStatusUser as any)?.isAktif
                                ? `Menonaktifkan status aktif dan hak akses login untuk akun @${toggleStatusUser?.username} (${toggleStatusUser?.namaLengkap}).`
                                : `Mengaktifkan kembali status akun dan hak akses login untuk @${toggleStatusUser?.username} (${toggleStatusUser?.namaLengkap}).`}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Batalkan</AlertDialogCancel>
            ...
            <Sheet open={openNew} onOpenChange={setOpenNew}>
                <SheetContent className="w-full sm:max-w-md overflow-y-auto">
                    ...
                    <div className="space-y-4 px-4 py-4 text-xs">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Select value={newRole} onValueChange={setNewRole}>
                                ...
                                <SelectContent className="text-xs max-h-56">
                                    {isSuperAdmin && (
                                        <>
                                            <SelectItem value="SUPER_ADMIN">Super Admin (MABES TNI AD)</SelectItem>
                                            <SelectItem value="ADMIN_KOTAMA">Admin Kotama / Balakpus</SelectItem>
                                        </>
                                    )}
                                    {isKotamaAdmin && (
                                        <SelectItem value="ADMIN_KOPERASI">Admin Satminkal / Koperasi</SelectItem>
                                    )}
                                    {!isSuperAdmin && !isKotamaAdmin && (
                        ...
                        {/* Kotama and Satminkal Placement for Form */}
                        {newRole === "SUPER_ADMIN" ? (
                            <div className="space-y-1.5">
                                <Label>Tingkat Organisasi</Label>
                                <div className="rounded-md border bg-muted/40 px-3 py-2 text-xs font-semibold text-foreground flex items-center gap-2">
                                    <Shield className="size-4 text-primary" />
                                    MABES TNI AD (Tingkat Pusat / Seluruh Indonesia)
                                </div>
                            </div>
                        ) : newRole === "ADMIN_KOTAMA" ? (
                            <div className="space-y-1.5">
                                <Label>Pilih Kotama / Balakpus <span className="text-destructive">*</span></Label>
                                <Select value={newKotamaId} onValueChange={setNewKotamaId}>
                                    <SelectTrigger>
                            ...
                            <>
                                ...
                                <div className="space-y-1.5">
                                    <Label>Satuan / Satminkal <span className="text-destructive">*</span></Label>
                                    <Select value={newSatminkalId} onValueChange={setNewSatminkalId}>
                                        <SelectTrigger>
                                            <SelectValue placeholder="Pilih Satminkal" />
                                        </SelectTrigger>
                                        <SelectContent className="max-h-56 text-xs">
                                            {availableSatminkalsForForm.map((s) => (
                                                <SelectItem key={s.id} value={s.id}>
                                                    {s.nama}
                                                </SelectItem>
                                            ))}
                            ...
                            <div className="space-y-1.5">
                                <Label>Satuan / Satminkal Binaan <span className="text-destructive">*</span></Label>
                                <Select value={newSatminkalId} onValueChange={setNewSatminkalId}>
                                    <SelectTrigger>
                                        <SelectValue placeholder="Pilih Satminkal" />
                                    </SelectTrigger>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <p className="text-[10px] text-muted-foreground">Satuan dinas — personel dialokasikan ke {satminkal}</p>
                            </div>
                        )}
            ...
            <AlertDialog open={openConfirmCreate} onOpenChange={setOpenConfirmCreate}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Akun login dan data anggota personel <strong>{newNamaLengkap}</strong> (NRP: {newNrpNip}) akan dibuat dan disinkronkan ke dalam sistem.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Batalkan</AlertDialogCancel>
...
export function ActiveSessionsWidget() {
    const queryClient = useQueryClient();
    const { isSuperAdmin, isKotamaAdmin } = useSession();
        ...
        <Card className="shadow-card mt-6">
            <CardHeader>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                        ...
                        <CardDescription className="text-xs mt-0.5">
                            {isSuperAdmin
                                ? "Pantau seluruh sesi admin Kotama dan Satminkal se-Indonesia. Super Admin memiliki wewenang Force Logout."
            ...
            <CardContent>
                        ...
                        <Table>
                            <TableHeader className="bg-muted/40">
                                <TableRow>
                                    ...
                                    <TableHead>Role Hak Akses</TableHead>
                                    {isSuperAdmin && <TableHead>Kotama / Balakpus</TableHead>}
                                    <TableHead>Satuan / Satminkal</TableHead>
                            ...
                            <TableBody>
                                {activeSessions.map((s: any) => {
                                    const kotamaName =
                                        s.kotama ||
                                        s.satminkalData?.kotama?.nama ||
                                        (s.role === "SUPER_ADMIN" ? "MABES TNI AD" : "-");
                ...
                <ConfirmActionDialog
                    ...
                    title="Putuskan Sesi Aktif Pengguna?"
                    description={
                        <>
                            Sesi aktif untuk pengguna <strong>@{sessionToTerminate?.username}</strong> ({sessionToTerminate?.namaLengkap}) akan diputus secara paksa. Pengguna akan segera dikeluarkan ke halaman login.
                        </>
                    }
