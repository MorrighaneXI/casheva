...
export function TopHeader() {
    ...
    // Check if this Kotama is actively being monitored by Super Admin
    const { data: activeKotamaMonitoring } = useQuery({
        queryKey: ["active-kotama-monitoring", kotamaId],
        queryFn: () => (kotamaId ? apiKotama.getActiveKotamaMonitoring(kotamaId) : Promise.resolve(null)),
        enabled: isKotamaAdmin && !isGuestMode && !!kotamaId,
    ...
    return (
        <>
            {/* 1. Banner Monitoring Super Admin -> Kotama */}
            {isGuestMode && monitoringKotama && (
                <div className="bg-rose-600 text-white px-4 py-2 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold sticky top-0 z-40 shadow-lg border-b border-rose-700">
                    <div className="flex items-center gap-2">
                        ...
                        <span>
                            🔴 <strong>MODE MONITORING (READ-ONLY)</strong>: Sedang mengamati Komando Utama <strong>{monitoringKotama.nama}</strong>. Seluruh aksi modifikasi data dinonaktifkan.
                        </span>
                    </div>
                    <Button
                        size="sm"
                        variant="secondary"
                        onClick={async () => {
                            await exitMonitoringKotama();
                            navigate({ to: "/" });
                            toast.info("Keluar dari Mode Monitoring Kotama. Kembali ke Portal Super Admin.");
                        }}
                        className="h-6 text-[11px] px-2.5 bg-white text-rose-700 hover:bg-rose-50 font-bold border-none shadow-sm"
                    >
                        ✕ Keluar ke Portal Super Admin
                    </Button>
                </div>
            )}

            {/* 2. Banner Monitoring Kotama -> Satminkal */}
            {isGuestMode && monitoringSatminkal && (
                <div className="bg-rose-600 text-white px-4 py-2 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold sticky top-0 z-40 shadow-lg border-b border-rose-700">
                    <div className="flex items-center gap-2">
                        <span className="flex h-2.5 w-2.5 relative">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
                        ...
                        <span>
                            🔴 <strong>MODE MONITORING (READ-ONLY)</strong>: Sedang mengamati Satminkal <strong>{monitoringSatminkal.nama}</strong>. Seluruh aksi modifikasi data dinonaktifkan.
                        </span>
                    </div>
                    <Button
                        size="sm"
                        variant="secondary"
                        onClick={async () => {
                            await exitMonitoring();
                            navigate({ to: "/" });
                            toast.info("Keluar dari Mode Monitoring. Kembali ke Dashboard Kotama.");
                        }}
                        className="h-6 text-[11px] px-2.5 bg-white text-rose-700 hover:bg-rose-50 font-bold border-none shadow-sm"
                    >
                        ✕ Keluar ke Dashboard Kotama
                    </Button>
                </div>
            )}

            {/* 3. Notifikasi Pengawasan Aktif untuk Admin Kotama yang sedang dipantau Super Admin */}
            {activeKotamaMonitoring?.isActive && !isGuestMode && isKotamaAdmin && (
                <div className="bg-amber-500 text-amber-950 px-4 py-2 flex flex-wrap items-center justify-between gap-2 text-xs font-bold sticky top-0 z-40 shadow-md border-b border-amber-600 animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="flex items-center gap-2">
                        <ShieldAlert className="size-4 text-amber-950 shrink-0" />
                        <span>
                            🛡️ <strong>PENGAWASAN PUSAT AKTIF</strong>: Komando Utama Anda ({kotama}) saat ini sedang dalam pengawasan langsung oleh <strong>{activeKotamaMonitoring.superAdmin?.namaLengkap || "Super Administrator Mabesad"}</strong> (Mode Tamu / Read-Only).
                        </span>
                    </div>
                    <Badge variant="outline" className="bg-amber-100/70 text-amber-950 border-amber-900/30 text-[10px] uppercase font-extrabold">
                        Dipantau Super Admin
                    </Badge>
                </div>
            )}
                ...
                <div className="hidden lg:flex items-center gap-1.5 px-3 py-1 rounded-lg border border-border/80 bg-muted/40 text-[11px] font-medium text-muted-foreground">
                    {isSuperAdmin && !isGuestMode ? (
                        <span className="font-bold text-primary flex items-center gap-1.5">
                            <ShieldCheck className="size-3.5" /> MABESAD / PUSAT (SUPER ADMIN)
                        </span>
                        ...
                        <span className="font-bold text-rose-600 dark:text-rose-400 flex items-center gap-1.5">
                            <Building2 className="size-3.5" /> {monitoringKotama.nama} (Monitoring)
                        </span>
                    ) : isKotamaAdmin && !isGuestMode ? (
                        <span className="font-semibold text-foreground">{kotama}</span>
                    ) : isKotamaAdmin && isGuestMode ? (
                        <>
                            <span className="font-semibold text-foreground">{satminkal}</span>
                            <span>/</span>
                            <span>{kotama} (Monitoring)</span>
                        </>
                    ) : (
                        <>
                            <span className="font-semibold text-foreground">{satminkal}</span>
                ...
                <DialogContent className="max-w-md">
                    ...
                    <div className="space-y-4 py-2 text-sm">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {isKotamaAdmin && !isGuestMode ? (
                                <div className="col-span-2 rounded-lg border border-border p-3 bg-card">
                                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                        <BadgeCheck className="size-3.5 text-primary" /> Komando Utama / Balakpus
                                    </div>
                                ...
                                <>
                                    <div className="rounded-lg border border-border p-3 bg-card">
                                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                            <Building2 className="size-3.5 text-primary" /> Satminkal {isGuestMode ? "(Monitoring)" : ""}
                                        </div>
